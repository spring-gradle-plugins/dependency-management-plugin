...
public class DependencyManagementContainer {
    ...
    /**
     * Creates a new {@code DependencyManagementContainer} that will hold dependency management for the given {@code
    ...
    void addImplicitManagedVersion(Configuration configuration, String group, String name,
            String version) {
    ...
    /**
     * Adds a managed version to the dependency management for the given {@code configuration}. The managed version
     * is for the dependency with the given {@code group}, {@code name}, and {@code version} and has the given {@code
     * exclusions}.
     *
    ...
    public Map<String, String> importedPropertiesForConfiguration(Configuration configuration) {
        ...
        if (configuration != null) {
            for (Configuration inHierarchy : getReversedHierarchy(configuration)) {
                properties.putAll(dependencyManagementForConfiguration(inHierarchy)
    ...
    /**
     * Returns the managed versions for the given {@code configuration} and its hierarchy. The returned map contains
     * keys of the form {@code groupId:artifactId}.
     *
     * @param configuration the configuration, or {@code null} for managed versions in global dependency management
    ...
    /**
     * Returns the managed versions for the given {@code configuration}. The returned map contains keys of the
     * form {@code groupId:artifactId}. If {@code inherited} is true, managed versions for the entire {@link
     * Configuration#getHierarchy() configuration hierarchy} are returned.
     *
     * @param configuration the configuration, or {@code null} for managed versions in global dependency management
     * @param inherited true if managed versions inherited from the configuration hierarchy should be returned
     * @return the managed versions for the configuration
     */
    public Map<String, String> getManagedVersionsForConfiguration(Configuration configuration, boolean inherited) {
        if (inherited) {
            ...
            if (configuration != null) {
                for (Configuration inHierarchy : getReversedHierarchy(configuration)) {
                    managedVersions.putAll(dependencyManagementForConfiguration(inHierarchy)
                            .getManagedVersions());
                }
            }
    ...
    private DependencyManagement dependencyManagementForConfiguration(Configuration configuration) {
        if (configuration == null) {
            ...
            DependencyManagement dependencyManagement = this.configurationDependencyManagement
                    .get(configuration);
