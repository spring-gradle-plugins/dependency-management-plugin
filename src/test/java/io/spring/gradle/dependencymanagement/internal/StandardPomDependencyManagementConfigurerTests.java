...
class StandardPomDependencyManagementConfigurerTests {
	...
	StandardPomDependencyManagementConfigurerTests() {
		...
		this.dependencyManagementContainer = new DependencyManagementContainer(this.project, this.pomResolver);
	}
	...
	private NodeAssert configuredPom(String existingPom, PomCustomizationSettings settings) throws Exception {
		...
		new StandardPomDependencyManagementConfigurer(dependencyManagement.getManagedDependencies(),
				...
				dependencyManagement.getResolvedBoms(new ProjectPropertySource(this.project)), settings)
			.configurePom(pom);
